  ...
  describe 'GET #show' do
    context 'with valid params' do
      ...
      it 'filters tasks by priority' do
        ...
        expect(assigns(:tasks)).to match_array(user.tasks.order(Arel.sql(priority) => :asc))
      end
      # 測試用 status 進行篩選
      it 'filters tasks by status' do
        status = tasks.first.status
        get :show, params: { id: user.id, status: }
      ...
      it 'filters tasks by title' do
        ...
        get :show, params: { id: user.id, title: }
      ...
      it 'sort by end_time and filters by status and title' do
        ...
        get :show, params: { id: user.id, sort_by:, title:, status: }
